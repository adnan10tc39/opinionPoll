...
def run_batch_chatbot(on_answer: Optional[Callable[[int, str, List[str]], None]] = None):
    """
    CLI / glue function.
    - Ask user for 1–5 questions.
    - Ask for 'max_per_platform' (posts to retrieve per platform).
    - Answer questions one-by-one; immediately emit each answer.

    Pass a callback `on_answer(idx, question, results)` to push each answer to your chatbot UI.
    """
    # Collect batch size (1–5)
    try:
        n = int(input("How many questions? (1–5): ").strip())
    except ValueError:
        n = 1
    n = max(1, min(5, n))
    ...
    for i in range(n):
        ...
        while not q:
            q = input(f"Q{i+1} (cannot be empty): ").strip()
        questions.append(q)

    # Posts per platform
    try:
        max_per_platform = int(input("Enter number of posts per platform (e.g., 3): ").strip())
    except ValueError:
        max_per_platform = 3

    print("\n--- Processing questions sequentially ---\n")
    for idx, q in enumerate(questions, start=1):
        results = perform_research(q, max_per_platform)

        # Emit immediately to console
        print(f"\n✅ Answer {idx}/{n} for: {q}\n")
        ...
        # Optional: emit to UI callback
        if on_answer:
            try:
                on_answer(idx, q, results if isinstance(results, list) else [str(results)])
            except Exception as e:
                # Non-fatal: just log and continue
                print(f"[on_answer callback error]: {e}")

if __name__ == "__main__":
    # Example: no history, no follow-ups — just batch Q&A
    run_batch_chatbot()
